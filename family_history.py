...
def print_death_age(people_dict):
    ...
    for key, value in people_dict.items():
        print(f'Name: {value[NAME_INDEX]}; age at death: {value[DEATH_YEAR_INDEX] - value[BIRTH_YEAR_INDEX]}')
...
def count_genders(people_dict):
    ...
    for key, value in people_dict.items():
        if value[GENDER_INDEX] == 'M': num_male += 1
        elif value[GENDER_INDEX] == 'F': num_female += 1
    print(f'There are {num_male} males and {num_female} females.')
...
def print_marriages(marriages_dict, people_dict):
    ...
    Return: nothing
    """
    for keyi, valuei in marriages_dict.items():
        husb_info = people_dict[valuei[HUSBAND_KEY_INDEX]]
        husb_name, husb_age = (husb_info[NAME_INDEX], valuei[WEDDING_YEAR_INDEX] - husb_info[BIRTH_YEAR_INDEX])
        wife_info = people_dict[valuei[WIFE_KEY_INDEX]]
        wife_name, wife_age = (wife_info[NAME_INDEX], valuei[WEDDING_YEAR_INDEX] - wife_info[BIRTH_YEAR_INDEX])
        print(f'In {valuei[WEDDING_YEAR_INDEX]}, {husb_name} and {wife_name} were married. He was {husb_age} and she was {wife_age}.')
