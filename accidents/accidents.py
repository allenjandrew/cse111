...
def main():
    ...
    while not got_file:
        ...
        try:
            ...
            print('Could not find file. Please enter a valid filename (with path, if necessary).')

        except PermissionError:
            print('You don\'t have permission to view this file. Please try again.')

        except csv.Error:
            'There is a problem with the CSV file. Exiting program...'
            break



        else: got_file = True # exit the loop
...
def estimate_reduction(row, behavior_key, perc_reduc):
    ...
    try: ratio = perc_reduc / 100 * behavior / fatal_crashes
    except ZeroDivisionError:
        ratio = 0
        print(f'No fatal crashes in {row[YEAR_COLUMN]}')
